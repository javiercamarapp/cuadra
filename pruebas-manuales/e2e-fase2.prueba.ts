...
describe('fase 2 · siembra + alta + cierre', () => {
  it('corre', async () => {
    const { crearFlota, crearOperador } = await import('@/lib/cuadra/administracion');
    const { guardarDatosFiscales, getDatosFiscales } = await import('@/lib/saas/fiscal');
    const { crearViaje, crearUnidad } = await import('@/lib/cuadra/operacion');
    const { addGasto, getGastos, gastoExistePorHash, saveLiquidacion } = await import('@/lib/cuadra/repo');
    const { subirComprobante } = await import('@/lib/cuadra/intake/almacen');
    const { hashImagen } = await import('@/lib/cuadra/intake/hash');
    const { decidirFoto } = await import('@/lib/cuadra/intake/decidir');
    const { decidirAcuse } = await import('@/lib/cuadra/acuse_ticket');
    const { cuadrarDesdeDB, ventanaDesdeDB } = await import('@/lib/cuadra/cuadre/desde_db');
    const { getConfig } = await import('@/lib/cuadra/config');
    const { generarLiquidacionPDF } = await import('@/lib/cuadra/liquidacion/pdf');
    const { supabaseAdmin } = await import('@/lib/supabase/admin');
    const { estadoDelViaje } = await import('@/lib/cuadra/consulta_chofer');
