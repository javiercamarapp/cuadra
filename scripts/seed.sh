...
echo ""
echo "   ⚠ Y para ENTRAR AL PANEL hace falta un paso más: esta base no tiene"
echo "     ningún usuario. El login no da de alta a nadie (shouldCreateUser:false),"
echo "     así que sin esto /login dice «te mandamos un link» y el link no llega:"
echo ""
echo "       node --env-file=.env.local scripts/crear-superadmin.mjs tu@correo.mx \"Tu nombre\""
echo ""
echo "     Detalle y variante para producción en DEPLOY.md → «El primer acceso al panel»."
