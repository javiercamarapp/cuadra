-- ═══════════════════════════════════════════════════════════════════════════
-- 0026 — `tenant.config` deja de ser un jsonb libre. Esquema en la base.
--
-- ALTO de la auditoría 5 (modelo de datos).
--
-- QUÉ ESTÁ MAL HOY. `tenant.config` (0004:8) es `jsonb` sin CHECK, y de él salen
-- TODOS los topes de dinero de una flota: la política de gastos, el tope fiscal
-- de $750/día de alimentación (LISR 28-V), el tope de $2,000 en efectivo
-- (LISR 27-III), el factor del estímulo de peaje. `getConfig` lo lee con un cast
-- ciego —`data?.config as Partial<CuadraConfig> | null` (config.ts:109)—, sin
-- zod, y `desde_db.ts:25` le entrega `config.politica` y `config.estimulos` al
-- motor tal cual.
--
-- Y ninguna línea de `src/` ESCRIBE esa columna: las tres apariciones de
-- `from('tenant')` (conv.ts:87, repo.ts:358, config.ts:147) son `select`. El
-- único escritor es una persona con la consola de Supabase abierta — el camino
-- que no pasa por ninguna validación.
--
-- EL ESCENARIO CON VALORES. Corrido contra el motor real (`cuadrarViaje` +
-- `fusionarConfig`) con alimentación $1,050 del 27-jul-2026, hospedaje $900 y
-- anticipo $2,000, cambiando SOLO la columna:
--
--   tenant.config                                          diferencias del motor
--   ─────────────────────────────────────────────────────  ───────────────────────
--   NULL (lo de hoy)                                       sobre_politica, anticipo, viatico_excede_fiscal
--   {"politica": []}                                       anticipo, viatico_excede_fiscal   ← se cayó el tope de la flota
--   {"estimulos":{"viaticosTopeFiscalDiarioMxn": null}}    sobre_politica, anticipo          ← se cayó el tope de LISR 28-V
--   {"politica": "si"}                                     TypeError: pol.filter is not a function
--
-- Las dos de en medio apagan un control fiscal SIN un log y SIN un error: la
-- liquidación sale diciendo que todo cumple. El caso del `null` es el más caro
-- porque el comentario de config.ts:65-87 documenta ese mismo fallo como
-- cerrado —lo cerró para el merge shallow, haciéndolo recursivo—, y escribir
-- `null` a mano en la columna lo reabre por otra puerta. La última mata el
-- "listo" del operador en producción.
--
-- QUÉ HACE ESTA MIGRACIÓN. Un CHECK que llama a una función de validación. La
-- función no devuelve `false`: LANZA con el motivo. Un `violates check
-- constraint "tenant_config_valida"` no le dice nada a quien está capturando la
-- config de una flota en la sala del demo; "el tope de $750/día viene en null y
-- eso apaga LISR 28-V" sí.
--
-- Las tres reglas que la sostienen:
--
--  1. Una llave que existe NO puede venir en `null`. `fusionarConfig` reemplaza
--     primitivos y null (config.ts:135), y el motor lee los topes con
--     `if (tope != null)`: null no es "usa el default", es "no apliques la
--     regla". Si lo que se quiere es el default, la llave se OMITE.
--  2. Una llave que el tipo no conoce se rechaza. `{"politicas": [...]}` —con
--     la "s"— hoy se guarda tan campante y la flota corre con los topes de
--     DEMO_CONFIG creyendo que corre con los suyos.
--  3. `politica` no puede ser un array VACÍO. Un array vacío quita el tope de
--     TODOS los conceptos y es indistinguible de una config a medio escribir.
--     Una flota que de verdad no quiera topes lo dice concepto por concepto,
--     como ya hace DEMO_CONFIG con `flete`: `[{"concepto":"flete"}]` es una
...
--
-- Reversible:
--   alter table tenant drop constraint tenant_config_valida;
--   drop function config_tenant_valida(jsonb);
-- ═══════════════════════════════════════════════════════════════════════════

create or replace function config_tenant_valida(p_config jsonb)
returns boolean
language plpgsql
immutable
as $fn$
declare
  -- Las 9 llaves de `CuadraConfig` (config.ts:22-52). Si aquí falta una que el
...
comment on function config_tenant_valida(jsonb) is
  'Esquema de tenant.config en la base. LANZA con el motivo en vez de devolver false: quien captura la config de una flota necesita saber QUÉ regla de dinero acaba de apagar, no que "violates check constraint".';

-- El CHECK. Idempotente por el `if not exists`; se valida contra las filas
-- actuales al aplicarlo (el único tenant tiene config NULL, así que pasa).
do $$
begin
  if not exists (
    ...
    alter table tenant
      ...
      check (config is null or config_tenant_valida(config));
  end if;
end $$;

comment on column tenant.config is
  'Override de CuadraConfig por flota. De aquí salen TODOS los topes de dinero. Validado por config_tenant_valida (0026): una llave presente no puede venir en null —eso apaga la regla en vez de usar el default— ni con un nombre que el tipo no conozca.';
