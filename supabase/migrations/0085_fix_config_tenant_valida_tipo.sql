-- 0085 — config_tenant_valida crasheaba al validar la facilidad del 15% (0083).
--
-- El CHECK `tenant_config_valida` corre en CADA insert/update de `tenant`, y la
-- 0083 le metió al bloque de la facilidad `r := o -> 'regimenElegible'` con `r`
-- declarada `record` (la misma variable que usan los loops de `estimulos` y
-- `tabulador` para iterar filas compuestas). Asignar un jsonb a una `record` es
-- "input of anonymous composite types is not implemented": con la facilidad
-- DECLARADA en `tenant.config`, cualquier UPDATE de cualquier columna del tenant
-- (nombre, RFC, domicilio, suscripción) tronaba — el alta y el panel no podían
-- guardar nada en una flota que ya declaró el 15%.
--
-- El fix usa una variable jsonb propia (`eleg`) para `regimenElegible`; la
-- `record` queda solo para los loops que sí iteran filas.
-- Reproducción (roja): update tenant set nombre=... where config tiene
-- facilidad → "input of anonymous composite types is not implemented".
