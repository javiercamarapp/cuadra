...
const enDecidir = (() => {
  ...
  const lineas = sinComentarios(readFileSync('src/lib/cuadra/intake/decidir.ts', 'utf8')).split('\n');
...
const enProcessor = (() => {
  const src = sinComentarios(readFileSync('src/lib/cuadra/processor.ts', 'utf8'));
