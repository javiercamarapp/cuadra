...
describe('getTenantContext', () => {
  ...
  it('control: con nombre, el del tenant', async () => {
    maybeSingle.mockResolvedValue({ data: { nombre: 'Transportes Innovativos' }, error: null });
    await expect(getTenantContext('t1')).resolves.toMatchObject({ nombreFlota: 'Transportes Innovativos' });
  });
});
