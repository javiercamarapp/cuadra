...
import { cuadrarViaje } from '@/lib/likida/cuadre/engine';
...
describe('la señal que lo distingue, y su límite', () => {
  const OCR = sinComentarios(readFileSync('src/lib/likida/intake/ocr.ts', 'utf8'));
...
describe('la nota no fiscal SÍ entra, pero avisa', () => {
  ...
  it('el motor levanta la diferencia y cita el 29-A', () => {
    const engine = readFileSync('src/lib/likida/cuadre/engine.ts', 'utf8');
...
describe('un voucher no es un acercamiento, aunque traiga código', () => {
  const FUENTE = sinComentarios(readFileSync('src/lib/likida/intake/ocr.ts', 'utf8'));
  ...
  it('y se le pide OTRO documento, no el mismo más completo', () => {
    const P = sinComentarios(readFileSync('src/lib/likida/processor.ts', 'utf8'));
