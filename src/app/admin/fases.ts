...
/**
 * El registro de fases que pinta `/admin/model-ops`: las TRES que de verdad
 * corren para cada viaje, en ese orden, con la prosa que explica qué hace cada
 * una. No hay una cuarta, ni un "crear agente nuevo": Likida no tiene
 * tool-calling configurable, son pasos fijos en código.
 *
 * Vive AQUÍ y no en la página porque era la QUINTA copia del mapa: repetía
 * `nombre` e `Icono` a mano, y era la copia que ya había divergido —solo tres
 * claves, así que `escalacion` salía en crudo ahí y con su nombre en las otras
 * tres pantallas—. El nombre y el ícono se derivan; lo único propio es
 * `queHace`. Auditoría 11 (arquitectura, G-46).
 */
export const FASES = ([
...
] as const).map((f) => ({ ...f, nombre: etiquetaFase(f.fase), Icono: ICONO_POR_FASE[f.fase] }));
